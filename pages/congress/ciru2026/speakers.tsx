import { CIRU2026_FACULTY } from "@/http/api";
...
const CIRU2026Speakers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        const res = await fetch(CIRU2026_FACULTY);
        if (!res.ok) {
  ...
  if (loading)
    return (
      <Loader src="/logos/ciru-logo.png" alt="Cargando speakers..." size={256} />
    );
      ...
      <div className="flex-shrink-0 px-4 py-3 bg-white border-b border-gray-100 shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-gray-100 text-sm text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#1B3B5F]/30"
          />
        </div>
      </div>
...
export default CIRU2026Speakers;
