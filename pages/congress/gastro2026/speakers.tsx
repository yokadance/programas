import { GASTRO2026_FACULTY } from "@/http/api";
...
const GASTRO2026Speakers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        const res = await fetch(GASTRO2026_FACULTY);
        if (!res.ok) {
    ...
    <div className="flex flex-col bg-gray-50" style={{ height: "100dvh" }}>
      ...
      <div className="flex-shrink-0 px-4 py-3 bg-white border-b border-gray-100 shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-gray-100 text-sm text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#3FA9CA]/30"
          />
        </div>
      </div>
      ...
      {selectedId && (
        <FacultyModal
          ...
          endpointUrl={GASTRO2026_FACULTY}
...
export default GASTRO2026Speakers;
