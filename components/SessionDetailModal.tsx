...
import { Session, Speaker } from "@/type/type";
...
const SessionDetailModal: React.FC<SessionDetailModalProps> = ({
  ...
  const cleanChairName = (chair: string) => {
    const parts = chair.split(",").map((p) => p.trim()).filter(Boolean);
  ...
  return (
    <>
      ...
      {/* Bottom sheet */}
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col md:inset-0 md:m-auto md:rounded-2xl md:max-w-2xl md:max-h-[90vh]">
        ...
        <div className="px-5 pt-3 pb-4 border-b border-gray-100">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              {/* Time + Location */}
              <div className={`flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium ${theme.primaryText} mb-1.5`}>
              ...
              {/* Title */}
              <h2 className={`text-base font-bold ${theme.titleText} leading-snug`}>
                {session.Session_Title}
              </h2>

              {/* Chair */}
              {session.Session_Chair && (
                <div className={`mt-2 inline-flex items-center gap-1.5 text-xs ${theme.lightBg} border ${theme.lightBorder} rounded-full px-3 py-1`}>
                  <UserRound className={`w-3 h-3 ${theme.chairIconColor} flex-shrink-0`} />
              ...
              {sortedPresentations.map((presentation, idx) => (
                <div
                  ...
                  {presentation.Start_Time && presentation.Start_Time !== "00:00" && (
                    <span className={`inline-flex items-center gap-1 text-xs font-semibold ${theme.primaryText} mb-1.5`}>
                  ...
                  <div className="flex items-start gap-2">
                    <ArrowRight className={`w-4 h-4 ${theme.iconColor} mt-0.5 flex-shrink-0`} />
                  ...
                  {presentation.AllSpeakers?.length > 0 && (
                    <div className="mt-2.5 pl-6 space-y-2">
                      {presentation.AllSpeakers.map((spk: Speaker, i: number) => (
                        <button
                          key={i}
                          ...
                          />
                          <div>
                            <p className={`text-sm font-semibold ${theme.primaryText} group-hover:underline`}>
                              {spk.Full_Name}
                            </p>
                            {spk.Country_Name && (
                              <p className="text-xs text-gray-400">{spk.Country_Name}</p>
                            )}
                          </div>
                        </button>
                      ))}
